...
author: Rahul Manna
date: 11 April 2025
date-format: "MMMM D, YYYY, 3:40-4:50"
...
::: {style="font-size: 60%;"}
Source: @ifvp_brain_loves_pictures
:::
...
## Effective Storytelling   {.smaller}
- Data alone doesn't engage people, **stories do**  
...
![](images/john_hopkins_covid.png){fig-align="center" width="80%"}

::: {style="font-size: 60%;"}
Source: @jhu_covid_map
:::
...
- Supports injury prevention by tracking workload 

Source @quickstart_sports_analytics
...
- **Effective Use of Color** → Use contrast wisely, avoid red-green, and use colorblind-friendly palettes. 
...
1. Visualization Packages in Python
2. Matplotlib Basics
3. Code Demo/Basic Examples
4. Advanced Examples
5. Other Packages
...
#### Altair (@altair_viz) {.smaller}  
- Declarative and user-friendly syntax for statistical plots  
- Built on **Vega-Lite**, optimized for simplicity  
- Great for visualizing structured data and trends  
...
- **Performance issues** – Slower for large datasets compared to specialized libraries.  
...
## Follow Along with Jupyter Notebooks

Download files in the `demo` folder from Github repo available at [github.com/ram200010/CSAS_2025_Data_Visualization](https://github.com/ram200010/CSAS_2025_Data_Visualization)

Link to GitHub Page for this workshop is also available on CSAS 2025 website.

You can also clone the repository or download the zip-file for the entire repo. 
...
x = np.linspace(-4,4,1000) # range of x values in plot

plt.figure(figsize=(6,4))
plt.plot(x,stand_norm(x),label='$N(\mu=0,\sigma=1)$')
...
plt.fill_between(x1,0,stand_norm(x1),alpha=0.5)
...
:::
::: {.column .center}
![Image courtesy of @aaron_judge_bio](images/aaron-judge.avif){width="60%"}
:::
::::
...
data = pd.read_csv('data/aaron_judge2024.csv')
...
plt.title('Aaron Judge Pitch Positions Over Home Plate',weight='bold',fontsize=16)
plt.xlabel('Horizontal Position (ft.)',weight='bold',fontsize=12)
plt.ylabel('Vertical Position (ft.)',weight='bold',fontsize=12)
plt.legend(ncols=2)
plt.show()
```
:::
